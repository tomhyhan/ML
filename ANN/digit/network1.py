...
class Network:
    ...
    def __init__(self, sizes):
        ...
        self.sizes = sizes
        self.weights = [np.random.rand(layer_j, layer_k) for layer_k, layer_j in zip(sizes[:-1], sizes[1:])]
    ...
    def update_mini_batch(self, mini_batch, learning_rate):
        ...
        for root_activations, validations in mini_batch:
            delta_nabla_w, delta_nabla_b = self.backprop(root_activations, validations)
        ...
        self.weights = [w - ((learning_rate / mini_batch_len) * nw) for w,nw in zip (self.weights, delta_nabla_w)]
        pass
    ...
    def evaluate(self, test_data):
        ...
        """
        testing_result = [(np.argmax(self.feedforward(image)), num) for image, num in test_data]
