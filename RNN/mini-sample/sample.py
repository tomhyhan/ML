...
# data I/O
data = open('input.txt', 'r').read().strip() # should be simple plain text file
chars = list(set(data))
data_size = len(data)
...
# model parameters
input_weights_U = np.random.randn(hidden_size, vocab_size) * 0.02     # input to hidden
hidden_weights_W = np.random.randn(hidden_size, hidden_size) * 0.02   # hidden to hidden
hidden_bias = np.zeros((hidden_size, 1)) # hidden bias

output_weights_V = np.random.randn(vocab_size, hidden_size) * 0.02    # hidden to output
...
while True:
  ...
  if n % 100 == 0 and p==0:
    sample_ix = sample(hidden_state_prev, inputs[0], data_size)
